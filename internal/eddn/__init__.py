...
class EDDNPlugin(Plugin):
    ...
    def load(self, config: Config) -> bool:
        ...
        self.actually_push = config.get("post", True)
    ...
    def check_system_name(
        journal: Journal, event: MutableMapping[str, Any], name: str
    ) -> bool:
        return journal.state["SystemName"] == event[name]

    @staticmethod
    def check_system_address(journal: Journal, event: MutableMapping[str, Any]) -> bool:
        return journal.state["SystemAddress"] == event["SystemAddress"]

    @staticmethod
    def check_star_pos(journal: Journal, event: MutableMapping[str, Any]) -> bool:
        return list(journal.state["StarPos"]) == event["StarPos"]

    @staticmethod
    def add_optional(message: dict, event: MutableMapping[str, Any], name: str) -> None:
        if name in event:
    ...
    def deep_pattern_removal(
        ...
        if isinstance(source, dict):
            return {
                k: EDDNPlugin.deep_pattern_removal(v, compiled, True)
                for k, v in source.items()
                if not any(r.search(k) for r in compiled)
            }
        elif isinstance(source, list):
            return [
                EDDNPlugin.deep_pattern_removal(item, compiled, True) for item in source
            ]
        else:
            return source
    ...
    def post_message(self, eddn: dict):
        ...
        if self.actually_push:
            response = httpx.post(self.eddn_url, json=eddn)
            if response.status_code != 200:
                logger.error(f"Failed to post to EDDN {response.text}")
        else:
            logger.info("EDDN message successfully validated (but not uploaded)")

    def __process_event(
        self, journal: Journal, event: MutableMapping[str, Any], eddn: dict
    ) -> bool:
        ...
        if event_type == "codexentry":
            ...
            message = {
                ...
                "commodities": [
                    ...
                    if not (
                        ("categoryname" in x and x["categoryname"] == "NonMarketable")
                        or ("legality" in x and x["legality"] != "")
                    )
                ],
            }
            ...
            message = {
                ...
                "odyssey": journal.state["Odyssey"],
                **self.pick_keys(
                    event, "MarketID", "StationName", "StationType", "Reason"
                ),
            }
            eddn["$schemaRef"] = self.schema_ref(
            ...
            message = {
                ...
                "odyssey": journal.state["Odyssey"],
                **self.pick_keys(
                    event, "MarketID", "StationName", "StationType", "LandingPad"
                ),
            }
            eddn["$schemaRef"] = self.schema_ref(
            ...
            message = {
                ...
                "horizons": journal.state["Horizons"],
                "odyssey": journal.state["Odyssey"],
                **self.pick_keys(event, "MarketID", "CarrierName", "CarrierID"),
            }

            if "Items" in event:
                message["Items"] = [
                    {**self.pick_keys(x, "id", "Name", "Price", "Stock", "Demand")}
                    for x in event["Items"]
                ]
            eddn["$schemaRef"] = self.schema_ref(
                "https://eddn.edcd.io/schemas/fcmaterials/1"
            )
            eddn["message"] = message
            ...
            message = {
                ...
                "StarPos": list(journal.state["StarPos"]),
                **self.pick_keys(
                    event, "timestamp", "SystemName", "SystemAddress", "Count"
                ),
            }
            eddn["$schemaRef"] = self.schema_ref(
            ...
            message = {
                ...
                "odyssey": journal.state["Odyssey"],
                **self.pick_keys(
                    event, "timestamp", "BodyName", "BodyID", "SystemAddress", "Signals"
                ),
            }
            eddn["$schemaRef"] = self.schema_ref(
            ...
            message = {
                ...
                "modules": [
                    x["Name"]
                    for x in event["Items"]
                    if re.search(self._RE_OUTFITTING_ITEMS, x["Name"], re.IGNORECASE)
                ],
            }
            eddn["$schemaRef"] = self.schema_ref(
            ...
            message = {
                ...
                "ships": [x["ShipType"] for x in event["PriceList"]],
            }
            eddn["$schemaRef"] = self.schema_ref(
    ...
    def __fss_signals(
        self, journal: Journal, event: MutableMapping[str, Any]
        ...
        if event_type == "fsssignaldiscovered":
            ...
            message = {
                ...
                "signals": [
                    ...
                    for x in signals
                    if x["SystemAddress"] == journal.state["SystemAddress"]
                    and (
                        "USSType" not in x or x["USSType"] != "USS_Type_MissionTarget;"
                    )
                ],
            }
    ...
    def journal_event(
        ...
        if entry is None or "event" not in entry:
            logger.error(f"Event missing from {entry}")
            return

        event_type = entry["event"].lower()
        # smush timestamp back into strings
        entry["timestamp"] = entry["timestamp"].isoformat()

        if (
            event_type == "startup"
        ):  # will be synthesized if game already running setup per journal state.
            self.__init_state(journal)

        event = self.deep_pattern_removal(entry, ["_Localised$"])
        ...
        eddn = {
            ...
            "header": {
                ...
                "softwareName": "Hugh EDDN Plugin",
                "softwareVersion": appversion(),
            },
            "message": {},
        }

        if event_type == "fsssignaldiscovered" or self.get_state(
            journal, "fss_signals"
        ):
            ...
            if message is not None:
                ...
                eddn["message"] = message
                pool.run0(self.post_message, eddn)
            if not leftover:
                return
    ...
    def config_screen(self, frame: tk.Frame, config: Config) -> None:
        post_var = tk.BooleanVar(value=self.actually_push)
        eddn_url = tk.StringVar(value=self.eddn_url)
        testing = tk.BooleanVar(value=self.testing)

        # Layout
        frame.columnconfigure(0, weight=1)

        # Actually Push
        ttk.Checkbutton(frame, text="Actually push to EDDN", variable=post_var).grid(
            row=0, column=0, sticky="w", pady=4
        )

        # EDDN URL
        ttk.Label(frame, text="EDDN Gateway URL:").grid(row=1, column=0, sticky="w")
        ttk.Entry(frame, textvariable=eddn_url, width=45).grid(
            row=2, column=0, sticky="ew", pady=4
        )

        # Testing mode
        ttk.Checkbutton(frame, text="Testing mode", variable=testing).grid(
            row=3, column=0, sticky="w", pady=4
        )

        def _auto_apply(*_):
            self.apply_config(config, post_var.get(), eddn_url.get(), testing.get())

        post_var.trace_add("write", _auto_apply)
        eddn_url.trace_add("write", _auto_apply)
        testing.trace_add("write", _auto_apply)

    def apply_config(
        self, config: Config, post: bool, eddn_url: str, testing: bool
    ) -> None:
        self.eddn_url = eddn_url
        self.testing = testing
        self.actually_push = post

        config.set("post", post)
        config.set("eddn_url", eddn_url)
        config.set("testing", testing)
